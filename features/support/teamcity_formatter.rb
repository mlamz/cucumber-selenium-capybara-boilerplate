class TeamCityFormatter
  ...
  def after_step_result(keyword, step_match, multiline_arg, status, exception, source_indent, background)
    if "#{status}".include?("undefined")
      test_failure("step: '#{step_match.name}' is undefined")
    end
  end
  ...
  def test_failure(msg, details='')
	...
	if details.include?("Component returned failure code: 0x80004005 (NS_ERROR_FAILURE)") == false
		@io.puts "##teamcity[testFailed #{timestamp} name='#{name}' message='#{msg}' details='#{details}']"
	else
		@io.puts "Firefix is complaining about Component returned failure code: 0x80004005. #{details}"
	end
